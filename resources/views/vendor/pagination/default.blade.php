@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation"
        ...
        <div class="flex-col hidden lg:flex-row sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
                <p class="text-sm leading-5 dark:text-gray-500">
                    {{ __('showing') }}
                    <span class="font-medium dark:text-gray-400">{{ $paginator->firstItem() }}</span>
                    {{ __('to') }}
                    <span class="font-medium dark:text-gray-400">{{ $paginator->lastItem() }}</span>
                    {{ __('of') }}
                    <span class="font-medium dark:text-gray-400">{{ $paginator->total() }}</span>
                    {{ __('results') }}
                </p>
            </div>
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span
                                            class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 text-white bg-blue-700 border border-blue-700 dark:text-white dark:border-secondary-600 dark:bg-secondary-700">{{ $page }}</span>
                                    </span>
                                @else
                                    {{-- Array Of Links Enable --}}
                                    <a href="{{ $url }}"
                                        class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 transition bg-white border border-gray-300 hover:text-gray-500 dark:hover:text-gray-500 focus:z-10 focus:outline-none focus:border-primary-300 focus:ring focus:ring-primary-300 focus:ring-opacity-30 dark:focus:ring-primary-500 dark:focus:ring-opacity-30 dark:text-gray-200 dark:border-secondary-600 dark:bg-secondary-700"
